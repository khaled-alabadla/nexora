...
| `inventory_movements` | Inventory | company_id, product_id, warehouse_id, type (`purchase\|sale\|return\|adjustment\|transfer_in\|transfer_out\|damage`), quantity `DECIMAL(18,4)`, unit_cost `DECIMAL(18,4)`, reference_type/reference_id (nullable), note, created_by, created_at only (**append-only, immutable**) | indexes on (company_id, product_id, warehouse_id), (warehouse_id), (type), (created_at) — no separate (company_id, product_id) index: it's a strict left-prefix of the composite, so it would add write overhead for zero query benefit (caught in 2.3 code review) |
...
| 2.4 | ✅ Adjustments & damage (2026-09-14) | `POST /inventory/adjustments`, negative-stock rules, FE adjustment form | permission boundary; negative-stock policy enforced end-to-end through the real endpoint; damage-sign validation; multi-line atomicity — 170 backend tests, 90 frontend tests, both gates green |
